...
function getcwd() {
    ...
    if (res < 0) {
        if (res === -platform_1.ERROR.ERANGE) {
...
function readdir(path, encoding) {
    ...
    /* Open directory. */
    var fd = files_1.open(path, platform_1.FLAG.O_RDONLY | platform_1.FLAG.O_DIRECTORY);
    if (fd < 0)
        throw fd;
...
function readdirList(path, encoding) {
    if (encoding === void 0) { encoding = 'utf8'; }
    var fd = files_1.open(path, platform_1.FLAG.O_DIRECTORY);
    if (fd < 0)
        throw fd;
    var buf = new Buffer(platform_1.PATH_MAX);
    var size = platform_1.linux_dirent64.size, unpack = platform_1.linux_dirent64.unpack;
...
function readdirListAsync(path, encoding, callback) {
    if (encoding === void 0) { encoding = 'utf8'; }
    files_1.openAsync(path, platform_1.FLAG.O_DIRECTORY, 0, function (fd) {
        if (fd < 0)
            return callback(fd);
